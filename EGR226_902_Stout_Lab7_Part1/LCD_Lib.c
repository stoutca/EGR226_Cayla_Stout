...
void LCD_init(void)
{
    commandWrite(0x3); //reset sequence
    delay_ms(10);
    commandWrite(0x3);
    delay_micro(200);
    commandWrite(0x3);
    delay_ms(10);
    commandWrite(0x2);//setting the 4-bit mode
    delay_micro(100);
    commandWrite(0x28); //setting it up for 2 lines, 5x7 format
    delay_micro(100);
    commandWrite(0x0F); //display on, cursor on, blinking
    delay_micro(100);
    commandWrite(0x01); //clear display, move cursor to home position
    delay_micro(100);
...
{
    P4->SEL0 &= ~BIT1;
    P4->SEL1 &= ~BIT1; //configure P4.1 as GPIO
    P4->DIR |= BIT1; //configure the direction register of P4.1 to be output

    P4->SEL0 &= ~BIT2;
    P4->SEL1 &= ~BIT2; //configure P4.2 as GPIO
    P4->DIR |= BIT2; //configure the direction register of P4.2 to be output
